...
DOCUMENTARY_TITLE_TERMS = {
    "과학적 비밀", "숨겨진 원리", "진짜 정체", "생기는 과정", "원리와 과정",
}
...
def validate_package(
    ...
    if not 170 <= narration_length <= 285:
    ...
    if len(script.midpoint_hook) < 8 or not 0.38 <= midpoint_ratio <= 0.72:
        raise QualityGateError("중간 반전 훅이 대본 중앙에 포함되지 않았습니다.")
    comedy_position = script.narration.find(script.comedy_beat)
    comedy_ratio = comedy_position / max(1, narration_length)
    if len(script.comedy_beat) < 8 or not 0.08 <= comedy_ratio <= 0.52:
        raise QualityGateError("주제에 맞는 B급 유머 한 줄이 대본 앞부분에 포함되지 않았습니다.")
    ...
    if not 8 <= len(script.engagement_question) <= 38 or not script.engagement_question.endswith(("?", "？")):
    ...
    blocked.extend(sorted(term for term in DOCUMENTARY_TITLE_TERMS if term in script.title))
    if blocked:
