"""AI로 주제를 고르고, 출처 범위 안에서 B급 교양 예능 대본을 작성한다."""
...
def normalize_loop_ending(narration: str, closing_loop: str) -> Tuple[str, str]:
    ...
    fallback = "이제 첫 질문이 조금 다르게 들립니다. 처음 장면을 다시 보면…"
...
조건:
- candidate_ids에는 후보 id만 넣고, 첫 2초의 궁금증·생활 공감·유머 보상·시각적 매력 순으로 최대 {max(1, limit)}개를 정렬한다.
- 최근 사용 주제와 비슷한 후보는 뒤로 보낸다.
- 딱딱한 자연 다큐보다 일상에서 바로 떠올릴 수 있고 한 줄 농담 뒤에 확실한 사실 보상이 있는 소재를 우선한다.
- 억지 유행어, 남을 조롱하는 소재, 음식·신체 현상을 혐오스럽게 표현해야만 웃긴 후보는 뒤로 보낸다.
...
"""
        ...
        for position, candidate_id in enumerate(ordered_ids[: max(1, limit)]):
            ...
            ranked.append(
                TopicPlan(
                    ...
                    audience_angle=selected.audience_angle,
                    comedy_angle=selected.comedy_angle,
                )
            )
        return ranked
    ...
    def write_script(
        ...
        prompt = f"""
당신은 한국어 B급 교양 예능 쇼츠의 작가다. 아래 '검증 자료'에 명시된 사실만 사용해 완전히 새 문장으로 대본을 작성한다.

주제: {plan.topic}
검증 자료 제목: {source.title}
검증 자료 URL: {source.url}
시청자 공감 각도: {plan.audience_angle}
허용된 유머 방향: {plan.comedy_angle}
검증 자료 본문:
{source.extract[:6000]}
{feedback_text}

작성 규칙:
- narration은 한국어 공백 포함 180~260자이며 33~45초 분량이다.
- 첫 문장은 2초 안에 이해되는 짧고 의외성 있는 질문형 hook과 같아야 한다. 인사와 배경 설명으로 시작하지 않는다.
- 첫 두 문장 안에 시청자 공감 상황과 핵심 의문을 모두 제시한다.
- comedy_beat는 주제에만 붙일 수 있는 무표정한 B급 유머 한 문장이다. narration의 10~35% 지점에 문장 그대로 넣는다.
- comedy_beat는 사실과 농담이 구분되어야 하며, 검증되지 않은 비유를 원인처럼 말하지 않는다.
- 억지 유행어, 인터넷 은어, 비속어, 혐오 표현, 성적 농담, 타인 조롱, 과도한 감탄사는 쓰지 않는다.
- 농담은 최대 두 번만 사용하고, 나머지는 2~4개의 구체적 사실을 원인→결과 흐름으로 빠르게 설명한다.
- midpoint_hook은 웃음 뒤에 오는 가장 강한 사실 보상 또는 관점 전환 한 문장이다. narration의 45~65% 지점에 문장 그대로 넣는다.
...
- engagement_question은 8~38자의 양자택일 또는 한 단어 답변형 질문이다. 지식을 시험하거나 긴 설명을 요구하지 않는다.
...
- title은 38자 이내의 자연스러운 한국어 질문 또는 상황형 문장이다. #shorts와 '과학적 비밀', '숨겨진 원리', '진짜 정체' 같은 다큐형 상투어를 넣지 않는다.
- description_intro는 생활 속 의문과 확인된 답을 소개하는 짧은 2문장이다.
- tags는 한국어 중심의 관련 태그 5~8개이며 # 기호는 제외한다.
- 교과서식 정의나 기계적 목록 낭독이 아니라 짧은 상황극처럼 시작해 사실로 보상하는 이야기로 쓴다.
...
"""
        ...
        for raw in result.get("tags", []):
            ...
            if tag and tag not in tags:
                tags.append(tag[:30])
        for fallback in (plan.topic, plan.category, "한입지식", "웃긴지식", "생활상식"):
        ...
        return ScriptPackage(
            ...
            comedy_beat=re.sub(r"\s+", " ", str(result["comedy_beat"])).strip(),
            tags=tags[:8],
        )
    ...
    def translate_caption_chunks(self, chunks: Iterable[str]) -> List[str]:
        ...
        for attempt in range(2):
            prompt = f"""
당신은 한국어 B급 교양 예능 쇼츠의 영문 자막 편집자다.
...
- 농담의 무표정한 어조를 살리되 영어권 OTT 예능 자막처럼 간결하고 자연스럽게 쓴다.
...
"""
    ...
    def review_script(
        ...
        """업로드 전에 사실성·재미·문장 품질·화면 적합성을 재검토한다."""
        prompt = f"""
당신은 한국어 B급 교양 예능 쇼츠의 최종 편집자다. 검증 자료와 대본을 대조해 냉정하게 검수한다.
...
- 첫 질문은 2초 안에 이해되고, 이 주제에만 붙일 수 있어야 한다.
- comedy_beat는 주제와 직접 연결된 한 줄이어야 하고, 억지 유행어·조롱·혐오·비속어 없이 실제로 미소가 날 정도여야 한다.
- 교과서 정의, 사실 목록, 다큐멘터리 문체가 2문장 이상 이어지면 entertaining을 false로 평가한다.
- 짧은 문장과 자연스러운 호흡으로 친근한 교양 예능 진행자가 읽기 편해야 한다.
- 중간 전환은 가장 강한 사실 보상을 주고, 결말은 첫 장면으로 자연스럽게 이어져야 한다.
- 스톡 영상으로 장면을 구성할 수 있어야 하며 과장·낚시·구독 요청이 없어야 한다.
- entertainment_score는 첫 2초 훅, 생활 공감, 유머 자연스러움, 전개 속도를 합쳐 0~100으로 평가한다.
- score는 사실성 40, 재미 25, 가독성 20, 화면 적합성 15의 비중으로 평가한다. 사람이 바로 공개해도 될 85점 이상만 승인한다.
"""
        schema = {
            "type": "object",
            "properties": {
        ...
        entertaining = bool(result.get("entertaining"))
        entertainment_score = max(
            0, min(100, int(result.get("entertainment_score", 0) or 0))
        )
        issues = [str(item).strip() for item in result.get("issues", []) if str(item).strip()]
        approved = bool(result.get("approved")) and score >= 85 and entertainment_score >= 70
        approved = approved and facts_supported and natural_korean and visualizable and entertaining
        return {
            "approved": approved,
            "score": score,
