...
무료 Pexels 촬영 배치에서 같은 검수 완료 성인 주 모델의 실제 연속동작만 선별해, 미국·글로벌용 영어 필라테스 쇼츠를 제작하고 YouTube에 공개합니다. 인종은 선발 조건으로 사용하지 않으며 유료 영상 생성 서비스와 배경음악도 사용하지 않습니다.

## 현재 운영 형식

- 활성 모델: `pexels-white-studio-black-v1`
- 영상: 1080×1920 실제 MP4, 세 동작, 약 20~46초
- 구도: 먼저 전체 자세를 보여준 뒤, 가슴은 어깨·팔꿈치·흉곽, 하체는 골반·고관절·무릎이 함께 보이는 기술 확대를 적용
...
- 설명: 영어 동작명·횟수·안전 안내·Pexels 원본 페이지·라이선스 표시
- 공개 확인: 업로드 응답만 믿지 않고 `public` 상태와 YouTube 처리 완료를 다시 조회

## 모델·원본 고정

- `assets/instructor/pexels-white-studio-black-v1.json`에 정확한 원본 ID, 페이지, 다운로드 주소, SHA-256, 길이, 해상도, 구도 좌표와 사람 검수 메모를 저장
- 현재 공개 풀은 서로 겹치지 않는 원본 9개와 루틴 3개
- 원본 바이트, 해상도, 길이, 제공처, 모델 ID 또는 검수 승인이 하나라도 달라지면 공개 중단
- 흰 스튜디오의 불투명 검정 운동복을 입은 같은 성인 주 모델만 사용하고 다른 출연자가 섞인 원본은 제외
- 원본 MP4는 저장소에 복제하지 않고 실행 때 Pexels에서 받아 해시를 확인
...
- 가슴·사타구니·엉덩이만 고립해 확대하지 않고, 근육 움직임과 연결된 인접 관절을 항상 함께 표시
...
`PEXELS_API_KEY`와 `PIXABAY_API_KEY`는 신규 무료 후보 검수 도구용입니다. 현재 고정 모델 공개 경로는 매니페스트에 잠근 Pexels 원본 URL과 SHA-256을 사용합니다.
