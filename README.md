# 한입지식 B급 교양 예능 Shorts Automation

타인의 인기 영상을 다시 올리지 않고, **생활 속 황당한 의문을 B급 유머와 검증된 사실로 푸는 원본 한국어 쇼츠를 하루 1편 자동 제작·업로드**하는 무료 MVP입니다.

## 무엇이 달라졌나

1. 한국 YouTube의 전체·코미디·생활·과학 인기 신호를 참고합니다. 다른 영상의 제목, 농담, 화면은 복제하지 않습니다.
2. 사람이 미리 연결한 생활밀착형 주제·유머 각도·위키백과 문서·스톡 검색어 목록 안에서만 오늘의 소재를 고릅니다.
3. 검색 결과를 추측하지 않고 등록된 위키백과 문서를 제목으로 직접 가져옵니다.
4. `2초 질문 → 생활 공감 → 무표정한 B급 유머 → 강한 사실 반전 → 처음으로 이어지는 결말` 구조로 33~45초 대본을 작성합니다.
5. 별도의 AI 편집자가 사실성·유머 자연스러움·한국어 가독성·화면 적합성을 재검수하며, 종합 85점과 재미 70점 이상만 승인합니다.
6. Pexels 또는 Pixabay의 주제별 스톡 영상, 친근한 교양 예능 내레이션, 한·영 병행 자막으로 9:16 영상을 만듭니다. 화면은 4~6개 비트로 빠르게 전환합니다.
7. YouTube에 하루 1편 공개하고, 기존 영상의 조회·좋아요·댓글 성과를 다음 주제 순위에 반영합니다.
8. 출처와 영상 제공자를 설명란에 자동 표시하고, 합성 콘텐츠 여부도 YouTube에 공개합니다.
...
- 검증 문서 제목이 편집 목록과 다르거나 종합 85점·재미 70점 기준을 넘지 못하면 다른 주제로 자동 전환
- 다큐형 상투어, 교과서식 정의 나열, 억지 유행어, 비속어·혐오·타인 조롱 차단
- 내레이션과 최종 영상 길이를 자동 확인
- 친근한 교양 예능 톤의 Gemini 음성을 우선 사용하고 장애 시 한국어 신경망 음성으로 자동 전환
- 전자음으로 들리던 합성 배경음은 사용하지 않고 음성 중심으로 믹싱
- 큰 한글과 간결한 영어를 반투명 안전 영역 패널에 함께 표시하며, 번역 장애 시 한글 자막으로 계속 운영
